...
class RemoveUnusedAccounts extends Maintenance {
	public function __construct() {
		...
		$this->addOption( 'email', 'Only remove accounts which email matches this', false, true );
	}
	...
	public function execute() {
		...
		if ( $this->hasOption( 'email' ) )
			$emails = explode(',', $this->getOption( 'email' ));
		else
			$emails =  ['test%neayi.com',
						'0hio0ak.com', 
						'biumemail.com', 
						'drypipe.com', 
						'genericimages.com', 
		...
		foreach ($emails as $email)
			$condParts[] = "(user_email LIKE '%".trim($email)."%')";
